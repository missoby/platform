...
// liste des commercant
    function afficheComm($offset = 0) {
        ...
        $res = $this->admin_model->getcommparent($this->limit, $offset)->result();
        if ($res) {
            ...
            foreach ($res as $row) {
                ...
                foreach ($res2 as $row2) {
                    $this->table->add_row($row->login, $row->email, $row->active, $row2->societe, $row2->enable, anchor('admin/admin/viewdetails/' . $row->idpersonne, '<i class="icon-eye-open"></i>', 'title="Voir Profil" class= "btn"') .
    ...
    function affichproduit($id, $offset = 0 ) {
        ...
        //générer la pagination 
            ...
            //generate pagination
            $this->load->library('pagination');
            $config['base_url'] = site_url('admin/admin/affichproduit/'.$id.'/');
            $config['total_rows'] = $this->admin_model->count_all_produit($id);
            $config['per_page'] = $this->limit;
            $config['uri_segment'] = $uri_segment;
            $this->pagination->initialize($config);
            $data['pagination'] = $this->pagination->create_links();
        ...
        // load data
        $products = $this->produit_model->get_paged_list_active($id ,$this->limit, $offset)->result();
        ...
        foreach ($products as $product) {
            $this->table->add_row($product->libelle, $product->stock, $product->prix, $product->remise, anchor('admin/admin/viewproduct/' . $product->idproduit, '<i class="icon-eye-open"></i>', 'title="Voir Produit" class= "btn"') . ' ' .
    ...
    function afficheClient($offset = 0,$logclient = NULL, $idnotif = NULL) {
        ...
        $res = $this->admin_model->getclientparent($this->limit, $offset)->result();
        if ($res) {
            //générer la pagination 
            // offset
            $uri_segment = 4;
            ...
            $config['total_rows'] = $this->admin_model->count_all_client();
            $config['per_page'] = $this->limit;
            $config['uri_segment'] = $uri_segment;
            $this->pagination->initialize($config);
            $data['pagination'] = $this->pagination->create_links();
            ...
            foreach ($res as $row) {
                ...
                foreach ($res2 as $row2) {
                    $this->table->add_row($row->nom, $row->prenom, $row->login, $row->pays, $row->ville, $row->tel, $row->email, $row->active, anchor('admin/admin/deleteclient/' . $row->idpersonne, '<i class="icon-trash"></i>', 'title="Supprimer client" class= "btn"', array('onclick' => "return confirm('Vous voulez vraiment supprimer ce produit?')"))
                    );
                }
            }
            $data['table'] = $this->table->generate();
